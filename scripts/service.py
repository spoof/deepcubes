...
print("Open log file...")
logging.basicConfig(filename='scripts/logs/log.txt',
...
def get_models_ids(path):
    models = [f_name for f_name in os.listdir(models_storage) if
              ('pickle' in f_name)]
    models_ids = map(lambda m: int(m.split('model-')[1].split('.pickle')[0]),
                     models)
...
def answer():
    ...
    if int(model_id) in get_models_ids(models_storage):
        classifier.load(
    ...
    logging.info('predicting intent for question: {}'.format(
...
def train():
    ...
    new_model_id = get_models_ids(models_storage)[-1] + 1
